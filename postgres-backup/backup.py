...
def main():
    ...
    run_pg_dump(os.environ["DATABASE_URL"], dump_path)
    compress_file(dump_path, gz_path)

    client = get_r2_client()
    bucket = os.environ["R2_BUCKET_NAME"]
    upload_to_r2(client, bucket, gz_path, remote_key)
    cleanup_old_backups(client, bucket)

    # Limpeza local -- o container do cron job e efemero mesmo, mas
    # nao custa nada liberar o espaco explicitamente.
    for path in (dump_path, gz_path):
